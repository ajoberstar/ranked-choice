...
(defrecord RankedChoice
  ...
  (count-ballots [vsys candidates ballots]
    (let [base-results (zipmap candidates (repeat []))
          ...
          votes-ch (async/chan 1 (comp (xf/peek (partial println "Votes In: "))
                                       (tally-xf tally)
                                       (xf/peek (partial println "Votes Out: "))))
          runoff-ch (async/chan 1 (comp (xf/peek (partial println "Runoff In: "))
                                        (take-while no-winner?)
                                        (xf/peek (partial println "Runoff Mid: "))
                                        (map runoff)
                                        (xf/peek (partial println "Runoff Out: "))))
          results-ch (async/chan 1 (comp (xf/peek (partial println "Results In: "))
                                         (map :tally)
                                         (map #(into base-tally %))
                                         (xf/peek (partial println "Results Out: "))))
